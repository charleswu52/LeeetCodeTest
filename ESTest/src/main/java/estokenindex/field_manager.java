...
public class field_manager {
    ...
    public void getEveryFieldInvertedListSize() {
        Iterator<Map.Entry<String, index_manager_v4_0_1>> iterator = _field_map.entrySet().iterator();
        while (iterator.hasNext()) {
            Map.Entry<String, index_manager_v4_0_1> next = iterator.next();
            String key = next.getKey();
            long invertedListSize = next.getValue().getInvertedListSize();
            System.out.println("当前文件："+key+"裸倒排大小："+ RamUsageEstimator.humanReadableUnits(invertedListSize));
        }
    }
}
