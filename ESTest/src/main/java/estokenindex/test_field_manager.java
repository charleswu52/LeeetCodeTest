...
//        ofs.close();
    ...
    void test_invert_list_size(String filePath) throws IOException {
        field_manager manager = new field_manager();
        manager.push_field_dir(filePath);

        manager.getEveryFieldInvertedListSize();
...
//        new test_field_manager().test_query_group(field_dir, field, query_path, union_result_path, intersection_result_path);
        new test_field_manager().test_invert_list_size("ESTest/src/main/resources/es_token_index/");
