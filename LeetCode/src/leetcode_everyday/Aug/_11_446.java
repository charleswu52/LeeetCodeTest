...
/**
 * @author WuChao
 * @create 2021/8/15 18:43
 */
public class _11_446 {
    /**
     * 每日一题：2021/8/11
     * 446. 等差数列划分 II - 子序列
     * 难度：hard
    ...
    由于尾项和公差可以确定一个等差数列，因此我们定义状态 nums[i]，公差为 d的弱等差子序列的个数。

    我们用一个二重循环去遍历 nums 中的所有元素对 (nums[i],nums[j])，其中 j<i。将 nums[i] 和 nums[j]
    分别当作等差数列的尾项和倒数第二项，则该等差数列的公差 d=nums[i]−nums[j]。由于公差相同，我们可以将 nums[i]
    加到以 nums[j] 为尾项，公差为 d 的弱等差子序列的末尾，这对应着状态转移 f[i][d] += f[j][d]。
    同时，(nums[i],nums[j]) 这一对元素也可以当作一个弱等差子序列，故有状态转移  f[i][d] += f[j][d] + 1
