...
public class _1 {
    ...
    /*
    思路：
    不需要具体关注每天到底吃几个糖果，只有思考两个情况就可以了
    逆向思维：在每天只吃一个|吃上限（capacity）个糖果的数量下不能保证在喜欢的天数时吃到喜欢吃的糖果类型
    1.一天吃一个，到那天时已经吃多了把想吃的吃完啦(onePerDay > preSum[type+1])
    2.一天吃上限个，到那天时之前的种类的糖果还是没有吃完(capPerDay <= preSum[type])
    符合上面两个场景下的一种就不会符合条件，因此当这两个条件都不满足时，才能保证在那天吃到想吃的糖果

     */
    public boolean[] canEat(int[] candiesCount, int[][] queries) {
        int queryLen = queries.length;
        boolean[] ans = new boolean[queryLen];
        int len = candiesCount.length;
        long[] preSum = new long[len+1]; // 前缀和数组
        preSum[0] = 0;
        for (int i = 0; i < len; i++) {
        ...
        for (int[] query : queries) {
            ...
            int onePerDay = day + 1;// 每天都吃一个
            long capPerDay = (long)cap * (day + 1); // 每天吃上限个
            ans[index++] = !(onePerDay > preSum[type+1] || capPerDay <= preSum[type]);
        }
        return ans;
